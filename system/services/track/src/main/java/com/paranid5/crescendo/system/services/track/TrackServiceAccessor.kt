...
internal class TrackServiceAccessor(
    ...
    private fun startTrackService(startType: TrackServiceStart) {
        Timber.tag(TAG).d("send start TrackService $startType")
    ...
    private fun switchPlaylist(startType: TrackServiceStart) {
        Timber.tag(TAG).d("send switch playlist")
    ...
    override fun addToPlaylist(track: DefaultTrack) {
        Timber.tag(TAG).d("Send add track $track to playlist")
    ...
    override fun removeFromPlaylist(trackInd: Int) {
        Timber.tag(TAG).d("Send remove track at $trackInd")
    ...
    override fun updatePlaylistAfterDrag() {
        Timber.tag(TAG).d("Send update playlist after drag")
...
internal fun Intent.putStartType(startType: TrackServiceStart) =
    apply { putExtra(TrackServiceBroadcasts.START_TYPE_ARG, startType) }

internal fun Intent.putTrack(track: DefaultTrack) =
